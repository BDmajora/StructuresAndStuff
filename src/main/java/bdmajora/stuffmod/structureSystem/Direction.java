...
public enum Direction {
	 ...
	 * EAST, 5, X
	 */
	X_POS(new BlockPosition(1, 0, 0), 5, "EAST", Axis.X),
	/**
	 * WEST, 4, X
	 */
	X_NEG(new BlockPosition(-1, 0, 0), 4, "WEST", Axis.X),
	/**
	 * UP, 1, Y
	 */
	Y_POS(new BlockPosition(0, 1, 0), 1, "UP", Axis.Y),
	/**
	 * DOWN, 0, Y
	 */
	Y_NEG(new BlockPosition(0, -1, 0), 0, "DOWN", Axis.Y),
	/**
	 * SOUTH, 3, Z
	 */
	Z_POS(new BlockPosition(0, 0, 1), 3, "SOUTH", Axis.Z),
	/**
	 * NORTH, 2, Z
	 */
	Z_NEG(new BlockPosition(0, 0, -1), 2, "NORTH", Axis.Z);

	private final BlockPosition vec;
	private Direction opposite;
	private final int side;
	private final String name;
	private final Axis axis;

	Direction(BlockPosition vec3I, int side, String name, Axis axis) {
		this.vec = vec3I;
		this.side = side;
		this.name = name;
		this.axis = axis;
	...
	public TileEntity getTileEntity(WorldSource world, TileEntity tile) {
		BlockPosition pos = new BlockPosition(tile.x + vec.x, tile.y + vec.y, tile.z + vec.z);
	...
	public Block getBlock(WorldSource world, BlockPosition baseVec) {
		BlockPosition pos = new BlockPosition(baseVec.x + vec.x, baseVec.y + vec.y, baseVec.z + vec.z);
		return world.getBlock(pos.x, pos.y, pos.z);
	}

	public TileEntity getTileEntity(WorldSource world, BlockPosition baseVec) {
		BlockPosition pos = new BlockPosition(baseVec.x + vec.x, baseVec.y + vec.y, baseVec.z + vec.z);
		return world.getTileEntity(pos.x, pos.y, pos.z);
	}
	...
	public Vec3 getMinecraftVec() {
		return Vec3.getPermanentVec3(vec.x, vec.y, vec.z);
	}
