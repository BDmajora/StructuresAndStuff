...
public class UtilIdRegistrar {
	private static int curr_item_id;
	private static int curr_block_id;
	...
	private static void throwException(int id) {
		if (usedIds[id]) {
			throw new RuntimeException("Tried to use id: " + id + " while it's already being used by " + Item.itemsList[id].getKey() + "!");
		} else {
			usedIds[id] = true;
		}
	}
